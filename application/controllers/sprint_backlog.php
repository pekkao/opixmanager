 ...
 * to handle Sprint_backlog listing, inserting, editing and deleting Sprint_backlogs.
...
class Sprint_Backlog extends CI_Controller
{
     ...
     * 
     * Constructor of a Sprint_backlog model. Loads Sprint_backlog_model and language package.
     */
    ...
    {   
        if ($this->session->userdata('logged_in'))
        {
            ...
            $data['sprint_backlogs'] = $this->sprint_backlog_model->read_all($productbacklogid);
            $data['currentproductbacklogid'] = $productbacklogid;
            $productbacklog = $this->product_backlog_model->read($productbacklogid);
            ...
            {
                $data['pagetitle'] = $project[0]->project_name . ', ' . $productbacklog[0]->backlog_name  . ': ' .
     ...
     * 
     * Creates an empty sprint backlog and shows it via sprint_backlog/sprint_backlogs_view.
     * @param int $currentproductbacklogid Primary key of the sprint backlog. 
     */
     ...
     * If no sprint backlog is found redirects to index with error message in flash data.
     * 
     * @param int $id, $currentproductbacklogid Primary key of the sprint backlog. 
     */
     public function edit($project_id, $id)
    {
        if ($this->session->userdata('logged_in'))
        {
            ...
            if (isset($sprint_backlog[0])) 
            {
                ...
                $data['project_id'] = $project_id; 
                $data['main_content'] = 'sprint_backlog/sprint_backlog_view';
                $data['pagetitle'] = $this->lang->line('title_edit_sprint_backlog');
            ...
            else 
            {
                ...
                redirect('sprint_backlog/index/' . $project_id . '/' . $currentproductbacklogid);
            }
        }
        else
        {
    ...
    public function save()
    {
        if ($this->session->userdata('logged_in'))
        {
            ...
            $data = array(
                ...
                'sprint_description' => $this->input->post('txt_sprint_description'),
            ...
            $this->form_validation->set_rules(
                    'txt_sprint_name', $this->lang->line('missing_name'), 'trim|required|max_length[255]|xss_clean');
            $this->form_validation->set_rules(
                    'txt_sprint_description', 'trim|max_length[1000]|xss_clean');
            $this->form_validation->set_rules(
                    'dtm_start_date', $this->lang->line('label_start_date'), 'required|check_date');
            $this->form_validation->set_rules(
                    'dtm_end_date', $this->lang->line('label_end_date'), 'check_date');
            ...
            else
            {
                if (!empty($data['start_date']) && !empty($data['end_date']))
                {
                    ...
                    else
                    {
                        if ($update == TRUE)
                        {
                            ...
                            $this->session->set_flashdata('$error_message', $error_message);
                            redirect('sprint_backlog/edit/' . $project_id . '/' . $data['id']);
                        }
                        else
                        {
                            $error_message = $this->lang->line('invalid_dates');
                            $this->session->set_flashdata('$error_message', $error_message);
                            redirect('sprint_backlog/add/' . $project_id . '/' . $data['product_backlog_id']);
                        }
                    }
                }
                else
                ...
                redirect('sprint_backlog/index/' . $project_id . '/' . $data['product_backlog_id']);
            }
        }
        else
        {
    ...
    public function delete()
    {
        ...
        if ($this->sprint_backlog_model->delete(intval($id)) == FALSE)
        {
            ...
            $this->session->set_flashdata('$error_message', $error_message);
            redirect('sprint_backlog/index/' . $project_id . '/' . $productbacklogid); 
        }
        else
        {
